...
# Log subfolder tag: <branch>/<osax-version>, so every build tree gets its own
# dir (/tmp/logs/yabai/<branch>/<ver>/) and a stale payload betrays itself by
# logging to an old version dir (the Godspeed build runner keys on this).
# Branch slashes sanitize to '_' so discovery stays a two-level walk.
YB_LOG_BRANCH := $(shell git -C $(CURDIR) rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '_')
ifeq ($(YB_LOG_BRANCH),)
YB_LOG_BRANCH := unknown
endif
YB_OSAX_VER := $(shell sed -n 's/^\#define OSAX_VERSION[[:space:]]*"\(.*\)"/\1/p' $(OSAX_PATH)/common.h)
ifeq ($(YB_OSAX_VER),)
YB_OSAX_VER := unknown
endif
YB_LOG_TREE := $(YB_LOG_BRANCH)/$(YB_OSAX_VER)
