...
void rule_serialize(FILE *rsp, struct rule *rule, int index) {
  ...
  fprintf(rsp,
          ...
          : title       ? title
                        : "",
          escaped_role ? escaped_role
          : role       ? role
                       : "",
          escaped_srole ? escaped_srole
          : srole       ? srole
                        : "",
          rule->effects.did
          ...
          rule->effects.opacity, json_optional_bool(rule->effects.manage),
          json_optional_bool(rule->effects.sticky),
          json_optional_bool(rule->effects.mff),
          rule_effects_check_flag(&rule->effects, RULE_LAYER)
              ? layer_str[rule->effects.layer]
              : "",
          json_optional_bool(rule->effects.fullscreen),
          rule_effects_check_flag(&rule->effects, RULE_MIN_WIDTH)
              ? rule->effects.min_width
              : 0,
          rule->effects.grid[0], rule->effects.grid[1], rule->effects.grid[2],
          rule->effects.grid[3], rule->effects.grid[4], rule->effects.grid[5],
...
void rule_combine_effects(struct rule_effects *effects,
                          struct rule_effects *result) {
  if (effects->did) {
  ...
  if (rule_effects_check_flag(effects, RULE_MIN_WIDTH) &&
      effects->min_width > 0) {
    result->min_width = effects->min_width;
    rule_effects_set_flag(result, RULE_MIN_WIDTH);
  }

  if (effects->scratchpad) {
    if (result->scratchpad)
      free(result->scratchpad);
  ...
  table_for(struct window * window, g_window_manager.window, {
    if (window->is_root) {
      ...
      window_manager_apply_manage_rules_to_window(
          &g_space_manager, &g_window_manager, window, window_title,
          window_role, window_subrole, false);
  ...
  table_for(struct window * window, g_window_manager.window, {
    if (window->is_root) {
      if (window_manager_rule_matches_window(
              rule, window, window_title_ts(window), window_role_ts(window),
              window_subrole_ts(window))) {
        window_manager_apply_manage_rule_effects_to_window(
            &g_space_manager, &g_window_manager, window, &rule->effects);
...
void rule_add(struct rule *rule) {
  if (rule->label)
    rule_remove_by_label(rule->label);
...
void rule_destroy(struct rule *rule) {
  if (rule_check_flag(rule, RULE_APP_VALID))
    regfree(&rule->app_regex);
  if (rule_check_flag(rule, RULE_TITLE_VALID))
    regfree(&rule->title_regex);
  if (rule_check_flag(rule, RULE_ROLE_VALID))
    regfree(&rule->role_regex);
  if (rule_check_flag(rule, RULE_SUBROLE_VALID))
    regfree(&rule->subrole_regex);
  ...
  if (rule->effects.scratchpad)
    free(rule->effects.scratchpad);
}
