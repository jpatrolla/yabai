...
static inline uint8_t mouse_mod_from_cgflags(uint32_t cgflags) {
  ...
  if ((cgflags & kCGEventFlagMaskAlternate) == kCGEventFlagMaskAlternate)
    flags |= MOUSE_MOD_ALT;
  if ((cgflags & kCGEventFlagMaskShift) == kCGEventFlagMaskShift)
    flags |= MOUSE_MOD_SHIFT;
  if ((cgflags & kCGEventFlagMaskCommand) == kCGEventFlagMaskCommand)
    flags |= MOUSE_MOD_CMD;
  if ((cgflags & kCGEventFlagMaskControl) == kCGEventFlagMaskControl)
    flags |= MOUSE_MOD_CTRL;
  if ((cgflags & kCGEventFlagMaskSecondaryFn) == kCGEventFlagMaskSecondaryFn)
    flags |= MOUSE_MOD_FN;
...
static MOUSE_HANDLER(mouse_handler) {
  ...
  case kCGEventTapDisabledByUserInput: {
    CFMachPortRef handle =
        __atomic_load_n(&mouse_state->handle, __ATOMIC_RELAXED);
    if (handle)
      CGEventTapEnable(handle, true);
  } break;
  case kCGEventLeftMouseDown: {
    // Ask CoreGraphics which window is under the pointer.
    CGWindowID wid = (CGWindowID)CGEventGetIntegerValueField(
        event,
        kCGMouseEventWindowUnderMousePointer); // ← field name
                                               // [oai_citation:0‡Apple
                                               // Developer](https://developer.apple.com/documentation/coregraphics/cgeventfield/mouseeventwindowundermousepointer?utm_source=chatgpt.com)

    // Fast-path: if we can’t map the ID to a yabai window we bail out.
    struct window *win = window_manager_find_window(&g_window_manager, wid);
    if (!win)
      break;

    // Rough heuristic: PiP windows always come from AVKit and have the
    // subrole "AXPictureInPictureWindow".  You may already have a helper
    // like `window_is_pip(win)` – otherwise add one that checks the
    // cached AXSubrole or window-level.
    // if (window_is_pip(win)) {
    //  window_exit_pip(win); // see §2
    //  return NULL;          // swallow the click so it doesn’t bubble
    //}
  }
...
void mouse_window_info_populate(struct mouse_state *ms,
...
enum mouse_drop_action mouse_determine_drop_action(struct mouse_state *ms,
                                                   struct window_node *src_node,
                                                   struct window *dst_window,
                                                   CGPoint point) {
  CGRect f = dst_window->frame;
  CGPoint wp = {point.x - f.origin.x, point.y - f.origin.y};
  CGRect c = {{0.25f * f.size.width, 0.25f * f.size.height},
              {0.50f * f.size.width, 0.50f * f.size.height}};
  CGPoint t[3] = {{0.0f, 0.0f},
                  {0.5f * f.size.width, 0.5f * f.size.height},
                  {f.size.width, 0.0f}};
  CGPoint r[3] = {{f.size.width, 0.0f},
                  {0.5f * f.size.width, 0.5f * f.size.height},
                  {f.size.width, f.size.height}};
  CGPoint b[3] = {{f.size.width, f.size.height},
                  {0.5f * f.size.width, 0.5f * f.size.height},
                  {0.0f, f.size.height}};
  CGPoint l[3] = {{0.0f, f.size.height},
                  {0.5f * f.size.width, 0.5f * f.size.height},
                  {0.0f, 0.0f}};

  if ((CGRectContainsPoint(c, wp)) && (src_node->window_count == 1)) {
    return ms->drop_action == MOUSE_MODE_STACK ? MOUSE_DROP_ACTION_STACK
...
void mouse_drop_action_stack(struct window_manager *wm, struct view *src_view,
                             struct window *src_window, struct view *dst_view,
                             struct window *dst_window) {
  space_manager_untile_window(src_view, src_window);
  window_manager_remove_managed_window(wm, src_window->id);

  struct window_node *dst_node =
      view_find_window_node(dst_view, dst_window->id);
  if (dst_node->window_count + 1 < NODE_MAX_WINDOW_COUNT) {
    ...
    scripting_addition_order_window(src_window->id, 1,
                                    dst_node->window_order[1]);

    if (dst_node->zoom) {
      window_manager_animate_window((struct window_capture){
          src_window, dst_node->zoom->area.x, dst_node->zoom->area.y,
          dst_node->zoom->area.w, dst_node->zoom->area.h});
    } else {
      window_manager_animate_window((struct window_capture){
          src_window, dst_node->area.x, dst_node->area.y, dst_node->area.w,
          dst_node->area.h});
    }
  }
}

void mouse_drop_action_swap(struct window_manager *wm, struct view *src_view,
                            struct window_node *src_node,
                            struct window *src_window, struct view *dst_view,
                            struct window_node *dst_node,
                            struct window *dst_window) {
  ...
  if (src_view->sid != dst_view->sid) {
    for (int i = 0; i < src_node->window_count; ++i) {
      ...
      window_manager_add_managed_window(
          wm, window_manager_find_window(wm, src_node->window_list[i]),
          src_view);
    }
    ...
    for (int i = 0; i < dst_node->window_count; ++i) {
      ...
      window_manager_add_managed_window(
          wm, window_manager_find_window(wm, dst_node->window_list[i]),
          dst_view);
    }
  }
...
void mouse_drop_action_warp(struct window_manager *wm, struct view *src_view,
                            struct window_node *src_node,
                            struct window *src_window, struct view *dst_view,
                            struct window_node *dst_node,
                            struct window *dst_window,
  ...
  if ((src_node->parent && dst_node->parent) &&
      (src_node->parent == dst_node->parent) && (src_node->window_count == 1)) {
    if (dst_node->parent->split == split) {
      mouse_drop_action_swap(wm, src_view, src_node, src_window, dst_view,
                             dst_node, dst_window);
      return;
    } else {
  ...
  struct window_node *src_node_rm =
      view_remove_window_node(src_view, src_window);
  window_manager_remove_managed_window(wm, src_window->id);
  window_manager_purify_window(wm, src_window);

  struct window_node *src_node_add = view_add_window_node_with_insertion_point(
      dst_view, src_window, dst_window->id);
  window_manager_add_managed_window(wm, src_window, dst_view);
...
void mouse_drop_no_target(struct space_manager *sm, struct window_manager *wm,
                          struct view *src_view, struct view *dst_view,
                          struct window *window, struct window_node *node) {
  if (src_view->sid == dst_view->sid) {
    ...
    struct view *view =
        space_manager_tile_window_on_space(sm, window, dst_view->sid);
    window_manager_add_managed_window(wm, window, view);
  }
}

void mouse_drop_try_adjust_bsp_grid(struct window_manager *wm,
                                    struct view *view, struct window *window,
                                    struct mouse_window_info *info) {
  bool success = true;
  ...
  if (info->changed_position) {
    ...
    if (window_manager_resize_window_relative(wm, window, direction, info->dx,
                                              info->dy, true) ==
  ...
  if (info->changed_size) {
    uint8_t direction = 0;
    if (info->changed_w && !info->changed_x)
      direction |= HANDLE_RIGHT;
    if (info->changed_h && !info->changed_y)
      direction |= HANDLE_BOTTOM;
    if (window_manager_resize_window_relative(wm, window, direction, info->dw,
                                              info->dh, true) ==
        WINDOW_OP_ERROR_INVALID_DST_NODE) {
      success = false;
    }
...
bool mouse_handler_begin(struct mouse_state *mouse_state, uint32_t mask) {
  if (mouse_state->handle)
    return true;

  mouse_state->handle = CGEventTapCreate(kCGHIDEventTap, kCGHeadInsertEventTap,
                                         kCGEventTapOptionDefault, mask,
                                         mouse_handler, mouse_state);
  if (!mouse_state->handle)
    return false;
  ...
  mouse_state->runloop_source =
      CFMachPortCreateRunLoopSource(NULL, mouse_state->handle, 0);
  CFRunLoopAddSource(CFRunLoopGetMain(), mouse_state->runloop_source,
...
void mouse_handler_end(struct mouse_state *mouse_state) {
  ...
  CFMachPortInvalidate(mouse_state->handle);
  CFRunLoopRemoveSource(CFRunLoopGetMain(), mouse_state->runloop_source,
                        kCFRunLoopCommonModes);
