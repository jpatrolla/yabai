...
void event_send_stack_indicator(uint32_t window_id, uint32_t stack_index) {
  mach_port_t port;
  if (g_bs_port && bootstrap_look_up(g_bs_port, "git.felix.jbevent", &port) ==
                       KERN_SUCCESS) {
    printf("[yabai debug] bootstrap lookup succeeded\n");

  } else {
    printf("[yabai debug] bootstrap lookup FAILED\n");
  }
  debug("[stack_indicator] sending: window_id=%d stack_indexxx=%d", window_id,
        stack_index);
  if (g_bs_port && bootstrap_look_up(g_bs_port, "git.felix.jbevent", &port) ==
                       KERN_SUCCESS) {
    struct {
    ...
    } data = {.event = 1337, .count = 1, .window_id = {0}, .stack_index = {0}};

    data.window_id[0] = window_id;
    data.stack_index[0] = stack_index;
    debug("sending this data:%d", data);
    mach_send(port, &data, sizeof(data));
  }
}
static bool event_signal_filter(struct event_signal *es,
  ...
  case SIGNAL_APPLICATION_VISIBLE: {
    int regex_match_app =
        signal->app_regex_exclude ? REGEX_MATCH_YES : REGEX_MATCH_NO;
    return regex_match(signal->app_regex_valid, &signal->app_regex, es->app) ==
           regex_match_app;
  } break;
  ...
  case SIGNAL_WINDOW_DESTROYED: {
    int regex_match_app =
        signal->app_regex_exclude ? REGEX_MATCH_YES : REGEX_MATCH_NO;
    bool app_no_match = regex_match(signal->app_regex_valid, &signal->app_regex,
                                    es->app) == regex_match_app;

    bool active = signal->active == SIGNAL_PROP_UD;
    if (!active)
      active = es->active == (signal->active == SIGNAL_PROP_YES);
  ...
  case SIGNAL_WINDOW_DEMINIMIZED: {
    ...
    int regex_match_title =
        signal->title_regex_exclude ? REGEX_MATCH_YES : REGEX_MATCH_NO;
    bool title_no_match =
        regex_match(signal->title_regex_valid, &signal->title_regex,
                    es->title) == regex_match_title;
...
void event_signal_flush(void) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    for (int j = 0; j < signal_count; ++j) {
      ...
      if (es->arg_name[0])
        setenv(es->arg_name[0], es->arg_value[0], 1);
      if (es->arg_name[1])
        setenv(es->arg_name[1], es->arg_value[1], 1);
      if (es->arg_name[2])
        setenv(es->arg_name[2], es->arg_value[2], 1);
      if (es->arg_name[3])
        setenv(es->arg_name[3], es->arg_value[3], 1);
...
void event_signal_push(enum signal_type type, void *context) {
  ...
  case SIGNAL_APPLICATION_TERMINATED: {
    ...
    //
    // NOTE(koekeishiya): We always receive an application_front_switched event
    // *before* an application_terminated event. We need to know the difference
    // between a front_switched + application_terminated sequence and a regular
    // application switch followed by a user-initiated termination of the
    // previously focused application. The system events are triggered within an
    // interval that appear to be impossible to match even with a user automated
    // sequence. The dt threshold below is triple the average interval computed,
  ...
  case SIGNAL_APPLICATION_FRONT_SWITCHED: {
    ...
    snprintf(es->arg_value[1], arg_size, "%d",
             g_process_manager.last_front_pid);
  } break;
  case SIGNAL_APPLICATION_HIDDEN: {
    struct application *application = context;
  ...
  case SIGNAL_WINDOW_DESTROYED: {
    ...
    es->app = window->application ? ts_string_copy(window->application->name)
                                  : "<unknown>";
  ...
  case SIGNAL_MISSION_CONTROL_EXIT: {
    enum mission_control_mode mode =
        (enum mission_control_mode)(uintptr_t)context;
...
enum signal_type signal_type_from_string(const char *str) {
  for (int i = SIGNAL_APPLICATION_LAUNCHED; i < SIGNAL_TYPE_COUNT; ++i) {
    if (string_equals(str, signal_type_str[i]))
      return i;
  }
...
static void event_signal_serialize(FILE *rsp, struct signal *signal,
                                   enum signal_type type, int index) {
  TIME_FUNCTION;
  ...
  fprintf(rsp,
          ...
          "\t\"action\":\"%s\"\n"
          "}",
          index, signal->label ? signal->label : "",
          ...
          : title       ? title
                        : "",
          json_optional_bool(signal->active), signal_type_str[type],
...
void event_signal_list(FILE *rsp) {
  ...
  for (int i = SIGNAL_APPLICATION_LAUNCHED; i < SIGNAL_TYPE_COUNT; ++i) {
    ...
    for (int j = 0; j < count; ++j) {
      ...
      if (j < buf_len(g_signal_event[i]) - 1)
        fprintf(rsp, ",");
      ++signal_index;
    }
