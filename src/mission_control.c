...
static CONNECTION_CALLBACK(connection_handler)
{
    if (type == 1204) {
        ...
        uint32_t wid; memcpy(&wid, data, sizeof(uint32_t));
        if (wid) event_loop_post(&g_event_loop, SLS_WINDOW_MOVED, (void *) (intptr_t) wid, 0);
    } else if (type == 807 ) {
        uint32_t wid; memcpy(&wid, data, sizeof(uint32_t));
        if (wid) event_loop_post(&g_event_loop, SLS_WINDOW_RESIZED, (void *) (intptr_t) wid, 0);
    } else if (type == 815) {
        uint32_t wid; memcpy(&wid, data, sizeof(uint32_t));
        if (wid) event_loop_post(&g_event_loop, SLS_WINDOW_VISIBLE, (void *) (intptr_t) wid, 0);
    } else if (type == 816) {
        uint32_t wid; memcpy(&wid, data, sizeof(uint32_t));
        if (wid) event_loop_post(&g_event_loop, SLS_WINDOW_INVISIBLE ,(void *) (intptr_t) wid, 0);
    } else if (type == 1325) {
        // NOTE: space-membership ADD payload (len 12) = { u64 sid; u32 wid at offset 8 }.
        uint32_t wid = 0;
        if (data && data_length >= 12) memcpy(&wid, (char *) data + 8, sizeof(uint32_t));
        if (wid) event_loop_post(&g_event_loop, SLS_ADDED_TO_SPACE, (void *) (intptr_t) wid, 0);
    } else if (type == 1326) {
        // NOTE: space-membership REMOVE, same payload as 1325. Does NOT fire on tab
        // de-select (ordered-out, not space-stripped); overlaps 804 on a real close.
        uint32_t wid = 0;
        if (data && data_length >= 12) memcpy(&wid, (char *) data + 8, sizeof(uint32_t));
        if (wid) event_loop_post(&g_event_loop, SLS_REMOVED_FROM_SPACE, (void *) (intptr_t) wid, 0);
    } else if (type == 1202) {
