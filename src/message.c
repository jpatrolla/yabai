...
extern char g_bsp_stack_indicator_position[8];
extern int g_bsp_stack_indicator_padding;
...
static const int token_char_int_table[] = {
    ...
    ['a'] = 0xA, ['A'] = 0xA, ['b'] = 0xB, ['B'] = 0xB, ['c'] = 0xC,
    ['C'] = 0xC, ['d'] = 0xD, ['D'] = 0xD, ['e'] = 0xE, ['E'] = 0xE,
    ['f'] = 0xF, ['F'] = 0xF,
};
...
static void parse_key_value_pair(char *token, char **key, char **value,
                                 bool *exclusion) {
  *key = token;
...
enum label_type { LABEL_DISPLAY, LABEL_SPACE, LABEL_WINDOW };

static char *reserved_display_identifiers[] = {
    ARGUMENT_COMMON_SEL_NORTH,  ARGUMENT_COMMON_SEL_EAST,
    ARGUMENT_COMMON_SEL_SOUTH,  ARGUMENT_COMMON_SEL_WEST,
    ARGUMENT_COMMON_SEL_PREV,   ARGUMENT_COMMON_SEL_NEXT,
    ARGUMENT_COMMON_SEL_FIRST,  ARGUMENT_COMMON_SEL_LAST,
    ARGUMENT_COMMON_SEL_RECENT, ARGUMENT_COMMON_SEL_MOUSE};
...
static char *reserved_window_identifiers[] = {
    ARGUMENT_WINDOW_TOGGLE_FLOAT,      ARGUMENT_WINDOW_TOGGLE_STICKY,
    ARGUMENT_WINDOW_TOGGLE_SHADOW,     ARGUMENT_WINDOW_TOGGLE_SPLIT,
    ARGUMENT_WINDOW_TOGGLE_PARENT,     ARGUMENT_WINDOW_TOGGLE_FULLSC,
    ARGUMENT_WINDOW_TOGGLE_WINDOWED,   ARGUMENT_WINDOW_TOGGLE_NATIVE,
    ARGUMENT_WINDOW_TOGGLE_EXPOSE,     ARGUMENT_WINDOW_TOGGLE_PIP,
    ARGUMENT_WINDOW_SCRATCHPAD_RECOVER};

static bool parse_label(FILE *rsp, struct token token, enum label_type type,
                        char **label) {
  ...
  case LABEL_DISPLAY: {
    for (int i = 0; i < array_count(reserved_display_identifiers); ++i) {
      if (token_equals(token, reserved_display_identifiers[i])) {
        daemon_fail(
            rsp,
            "'%.*s' is a reserved keyword and cannot be used as a label.\n",
  ...
  case LABEL_WINDOW: {
    for (int i = 0; i < array_count(reserved_window_identifiers); ++i) {
      if (token_equals(token, reserved_window_identifiers[i])) {
        daemon_fail(rsp,
                    "'%.*s' is a reserved keyword and cannot be used as a "
                    "scratchpad.\n",
                    token.length, token.text);
        return false;
      }
...
static inline bool parse_property(struct properties *properties, char *property,
                                  uint64_t *property_val, char **property_str,
                                  int property_count) {
...
static struct properties parse_properties(FILE *rsp, struct token token,
  ...
  if ((result.did_parse =
           token_is_valid(token) && !token_prefix(token, "--"))) {
    for (int i = 0, cursor = 0; i < token.length; ++i) {
      if (i + 1 == token.length) {
        if (!parse_property(&result, token.text + cursor, property_val,
                            property_str, property_count)) {
...
static struct selector parse_display_selector(FILE *rsp, char **message,
                                              uint32_t acting_did,
                                              bool optional) {
  TIME_FUNCTION;
  ...
  if (value.type == TOKEN_TYPE_INT) {
    ...
    if (did) {
      ...
      daemon_fail(rsp,
                  "could not locate display with arrangement index '%d'.\n",
                  value.int_value);
    }
  } else if (value.type == TOKEN_TYPE_STRING) {
    if (token_equals(result.token, ARGUMENT_COMMON_SEL_NORTH)) {
      if (acting_did) {
        uint32_t did = display_manager_find_closest_display_in_direction(
            acting_did, DIR_NORTH);
        if (did) {
          result.did = did;
        } else {
      ...
      struct display_label *display_label =
          display_manager_get_display_for_label(&g_display_manager,
                                                value.string_value);
      if (display_label) {
...
static struct selector parse_space_selector(FILE *rsp, char **message,
                                            uint64_t acting_sid,
                                            bool optional) {
  TIME_FUNCTION;
  ...
  if (value.type == TOKEN_TYPE_INT) {
    ...
    if (sid) {
      ...
      daemon_fail(rsp,
                  "could not locate space with mission-control index '%d'.\n",
                  value.int_value);
    }
  } else if (value.type == TOKEN_TYPE_STRING) {
    if (token_equals(result.token, ARGUMENT_COMMON_SEL_PREV)) {
      ...
      struct space_label *space_label = space_manager_get_space_for_label(
          &g_space_manager, value.string_value);
      if (space_label) {
        result.did_parse = true;
...
static struct selector parse_window_selector(FILE *rsp, char **message,
  ...
  if (value.type == TOKEN_TYPE_INT) {
    struct window *window =
        window_manager_find_window(&g_window_manager, value.int_value);
    if (window) {
    ...
    if (token_equals(result.token, ARGUMENT_COMMON_SEL_NORTH)) {
      if (acting_window) {
        struct window *closest_window =
            window_manager_find_closest_managed_window_in_direction(
                &g_window_manager, acting_window, DIR_NORTH);
        if (closest_window) {
      ...
      if (acting_window) {
        struct window *closest_window =
            window_manager_find_closest_managed_window_in_direction(
                &g_window_manager, acting_window, DIR_SOUTH);
        if (closest_window) {
          result.window = closest_window;
        } else {
      ...
      if (acting_window) {
        struct window *closest_window =
            window_manager_find_closest_managed_window_in_direction(
                &g_window_manager, acting_window, DIR_WEST);
        if (closest_window) {
          result.window = closest_window;
        } else {
      ...
      struct window *mouse_window =
          window_manager_find_window_below_cursor(&g_window_manager);
      if (mouse_window) {
      ...
      struct window *area_window = window_manager_find_largest_managed_window(
          &g_space_manager, &g_window_manager);
      if (area_window) {
      ...
      struct window *area_window = window_manager_find_smallest_managed_window(
          &g_space_manager, &g_window_manager);
      if (area_window) {
        result.window = area_window;
      } else {
      ...
      if (acting_window) {
        struct window *sibling_window =
            window_manager_find_sibling_for_managed_window(&g_window_manager,
                                                           acting_window);
        if (sibling_window) {
      ...
      if (acting_window) {
        struct window *nephew_window =
            window_manager_find_first_nephew_for_managed_window(
                &g_window_manager, acting_window);
        if (nephew_window) {
      ...
      if (acting_window) {
        struct window *uncle_window =
            window_manager_find_uncle_for_managed_window(&g_window_manager,
                                                         acting_window);
        if (uncle_window) {
      ...
      if (acting_window) {
        struct window *cousin_window =
            window_manager_find_first_cousin_for_managed_window(
                &g_window_manager, acting_window);
        if (cousin_window) {
      ...
      if (acting_window) {
        struct window *prev_window = window_manager_find_prev_managed_window(
            &g_space_manager, &g_window_manager, acting_window);
        if (prev_window) {
      ...
      if (acting_window) {
        struct window *next_window = window_manager_find_next_managed_window(
            &g_space_manager, &g_window_manager, acting_window);
        if (next_window) {
      ...
      struct window *first_window = window_manager_find_first_managed_window(
          &g_space_manager, &g_window_manager);
      if (first_window) {
      ...
      struct window *last_window = window_manager_find_last_managed_window(
          &g_space_manager, &g_window_manager);
      if (last_window) {
      ...
      struct window *recent_window =
          window_manager_find_recent_managed_window(&g_window_manager);
      if (recent_window) {
      ...
      if (acting_window) {
        ...
        if (token_equals(result.token, ARGUMENT_COMMON_SEL_PREV)) {
          struct window *prev_window = window_manager_find_prev_window_in_stack(
              &g_space_manager, &g_window_manager, acting_window);
          if (prev_window) {
            result.window = prev_window;
          } else {
          ...
          struct window *next_window = window_manager_find_next_window_in_stack(
              &g_space_manager, &g_window_manager, acting_window);
          if (next_window) {
            result.window = next_window;
          } else {
          ...
          struct window *last_window = window_manager_find_last_window_in_stack(
              &g_space_manager, &g_window_manager, acting_window);
          if (last_window) {
            result.window = last_window;
          } else {
          ...
          struct window *recent_window =
              window_manager_find_recent_window_in_stack(
                  &g_space_manager, &g_window_manager, acting_window);
          if (recent_window) {
            result.window = recent_window;
          } else {
            daemon_fail(rsp, "could not locate the recent stacked window.\n");
                   ...
                   token_is_positive_integer(result.token, &index) &&
                   index > 0) {
          struct window *index_window = window_manager_find_window_in_stack(
              &g_space_manager, &g_window_manager, acting_window, index);
          if (index_window) {
            result.window = index_window;
          } else {
            daemon_fail(rsp,
                        "could not locate the stacked window in position %d.\n",
                        index);
          }
        } else {
          result.did_parse = false;
          daemon_fail(rsp,
                      "value '%s%.*s' is not a valid option for WINDOW_SEL\n",
                      ARGUMENT_COMMON_SEL_STACK_PREFIX, result.token.length,
                      result.token.text);
        }
      } else {
        daemon_fail(rsp, "could not locate the selected window.\n");
...
static void handle_domain_config(FILE *rsp, struct token domain,
  ...
  if (found_selector) {
    struct selector space_selector =
        parse_space_selector(rsp, &message, 0, false);
  ...
  for (; token_is_valid(command); command = get_token(&message)) {
    if (token_equals(command, COMMAND_CONFIG_DEBUG_OUTPUT)) {
      ...
      if (!token_is_valid(value)) {
        ...
        window_manager_set_focus_follows_mouse(&g_window_manager,
                                               FFM_AUTOFOCUS);
      ...
      if (value.type == TOKEN_TYPE_INVALID) {
        ...
        daemon_fail(
            rsp,
            "unknown value '%.*s' given to command '%.*s' for domain '%.*s'\n",
            value.token.length, value.token.text, command.length, command.text,
            domain.length, domain.text);
      }
      ...
      if (value.type == TOKEN_TYPE_INVALID) {
        ...
        if (value.float_value == 0.0f) {
          ...
          daemon_fail(
              rsp,
              "command '%.*s' for domain '%.*s' requires System Integrity "
              "Protection to be partially disabled! ignoring request..\n",
              command.length, command.text, domain.length, domain.text);
        } else if (CGPreflightScreenCaptureAccess()) {
          g_window_manager.window_animation_duration = value.float_value;
        } else {
          daemon_fail(rsp,
                      "command '%.*s' for domain '%.*s' requires Screen "
                      "Recording permissions! ignoring request..\n",
                      command.length, command.text, domain.length, domain.text);
      ...
      if (!token_is_valid(value)) {
        fprintf(rsp, "%s\n",
                animation_easing_type_str[g_window_manager
        ...
        if (!match)
          daemon_fail(rsp,
                      "unknown value '%.*s' given to command '%.*s' for domain "
                      "'%.*s'\n",
                      value.length, value.text, command.length, command.text,
                      domain.length, domain.text);
      }
      ...
      if (value.type == TOKEN_TYPE_INVALID) {
        fprintf(rsp, "%.4f\n", g_window_manager.menubar_opacity);
                 ...
                 in_range_ii(value.float_value, 0.0f, 1.0f)) {
        window_manager_set_menubar_opacity(&g_window_manager,
                                           value.float_value);
      } else {
        daemon_fail(
            rsp,
      ...
      if (value.type == TOKEN_TYPE_INVALID) {
        fprintf(rsp, "%.4f\n", g_window_manager.active_window_opacity);
                 ...
                 in_range_ei(value.float_value, 0.0f, 1.0f)) {
        window_manager_set_active_window_opacity(&g_window_manager,
                                                 value.float_value);
      } else {
        daemon_fail(
            rsp,
      ...
      if (value.type == TOKEN_TYPE_INVALID) {
        fprintf(rsp, "%.4f\n", g_window_manager.normal_window_opacity);
                 ...
                 in_range_ei(value.float_value, 0.0f, 1.0f)) {
        window_manager_set_normal_window_opacity(&g_window_manager,
                                                 value.float_value);
      } else {
        daemon_fail(
            rsp,
      ...
      if (value.type == TOKEN_TYPE_INVALID) {
        ...
        g_window_manager.insert_feedback_color =
            rgba_color_from_hex(value.u32_value);
      } else {
        daemon_fail(
            rsp,
            "unknown value '%.*s' given to command '%.*s' for domain '%.*s'\n",
      ...
      if (sel_sid) {
        ...
        if (value.type == TOKEN_TYPE_INVALID) {
          ...
          daemon_fail(rsp,
                      ...
                      command.text, domain.length, domain.text);
        }
      } else {
        if (value.type == TOKEN_TYPE_INVALID) {
          fprintf(rsp, "%d\n", g_space_manager.top_padding);
        } else if (value.type == TOKEN_TYPE_INT) {
          space_manager_set_top_padding_for_all_spaces(&g_space_manager,
                                                       value.int_value);
        } else {
          daemon_fail(rsp,
                      "unknown value '%.*s' given to command '%.*s' for domain "
      ...
      if (sel_sid) {
        ...
        if (value.type == TOKEN_TYPE_INVALID) {
          ...
          space_manager_set_bottom_padding_for_all_spaces(&g_space_manager,
                                                          value.int_value);
        } else {
          daemon_fail(rsp,
                      "unknown value '%.*s' given to command '%.*s' for domain "
      ...
      if (sel_sid) {
        ...
        if (value.type == TOKEN_TYPE_INVALID) {
          ...
          space_manager_set_left_padding_for_all_spaces(&g_space_manager,
                                                        value.int_value);
        } else {
          daemon_fail(rsp,
                      "unknown value '%.*s' given to command '%.*s' for domain "
      ...
      if (sel_sid) {
        ...
        if (value.type == TOKEN_TYPE_INVALID) {
          ...
          space_manager_set_right_padding_for_all_spaces(&g_space_manager,
                                                         value.int_value);
        } else {
          daemon_fail(rsp,
                      "unknown value '%.*s' given to command '%.*s' for domain "
      ...
      if (sel_sid) {
        ...
        if (value.type == TOKEN_TYPE_INVALID) {
          ...
          space_manager_set_window_gap_for_all_spaces(&g_space_manager,
                                                      value.int_value);
        } else {
          daemon_fail(rsp,
                      "unknown value '%.*s' given to command '%.*s' for domain "
        ...
        if (!token_is_valid(value)) {
          ...
          if (space_is_user(sel_sid)) {
            ...
            window_manager_validate_and_check_for_windows_on_space(
                &g_space_manager, &g_window_manager, sel_sid);
          } else {
            daemon_fail(rsp,
      ...
      if (value.type == TOKEN_TYPE_INVALID) {
        fprintf(rsp, "%.4f\n", g_space_manager.split_ratio);
                 ...
                 in_range_ii(value.float_value, 0.1f, 0.9f)) {
      ...
      if (sel_sid) {
        ...
        if (!token_is_valid(value)) {
          ...
          space_manager_set_split_type_for_all_spaces(&g_space_manager,
                                                      SPLIT_Y);
        } else if (token_equals(value, ARGUMENT_CONFIG_SPLIT_TYPE_X)) {
          space_manager_set_split_type_for_all_spaces(&g_space_manager,
                                                      SPLIT_X);
      ...
      if (sel_sid) {
        ...
        if (!token_is_valid(value)) {
          ...
          space_manager_set_auto_balance_for_all_spaces(&g_space_manager,
                                                        SPLIT_NONE);
        } else if (token_equals(value, ARGUMENT_COMMON_VAL_ON)) {
          space_manager_set_auto_balance_for_all_spaces(&g_space_manager,
                                                        SPLIT_X | SPLIT_Y);
    ...
    else if (token_equals(command, "bsp_stack_indicators")) {
      struct token position = get_token(&message);
      struct token padding = get_token(&message);

      if (!token_is_valid(position) || !token_is_valid(padding)) {
        daemon_fail(rsp, "invalid arguments for 'bsp_stack_indicators'. Usage: "
                         "bsp_stack_indicators [left|top|off] <padding>\n");
      } else {
        snprintf(g_bsp_stack_indicator_position,
                 sizeof(g_bsp_stack_indicator_position), "%.*s",
                 position.length, position.text);

        struct token_value value = token_to_value(padding);
        if (value.type == TOKEN_TYPE_INT) {
          g_bsp_stack_indicator_padding = value.int_value;
        } else {
          daemon_fail(
              rsp, "invalid padding value '%.*s' for 'bsp_stack_indicators'\n",
              padding.length, padding.text);
        }
      }
      ...
      if (!token_is_valid(value)) {
        ...
        daemon_fail(rsp,
                    "unknown value '%.*s' given to command '%.*s' for "
                    "domain '%.*s'\n",
                    value.length, value.text, command.length, command.text,
                    domain.length, domain.text);
      }
      ...
      char mode[6];
      struct token value = get_token(&message);
      if ((sscanf(value.text, ARGUMENT_CONFIG_EXTERNAL_BAR, mode, &t, &b) ==
           3)) {
        if (string_equals(mode, ARGUMENT_CONFIG_EXTERNAL_BAR_MAIN)) {
          ...
          daemon_fail(rsp,
                      "unknown mode '%s' specified in value '%.*s' given "
                      "to command '%.*s' for domain '%.*s'\n",
                      mode, value.length, value.text, command.length,
                      command.text, domain.length, domain.text);
        }
      } else {
        fprintf(
            rsp, "%s:%d:%d\n", external_bar_mode_str[g_display_manager.mode],
            g_display_manager.top_padding, g_display_manager.bottom_padding);
      }
    } else {
...
static void handle_domain_display(FILE *rsp, struct token domain,
                                  char *message) {
  TIME_FUNCTION;
  ...
  struct selector selector =
      parse_display_selector(NULL, &message, acting_did, true);
  ...
  if (token_equals(command, COMMAND_DISPLAY_FOCUS)) {
    ...
    if (selector.did_parse && selector.sid) {
      ...
      if (result == SPACE_OP_ERROR_SAME_DISPLAY) {
        ...
        daemon_fail(rsp, "cannot focus space because the display is in the "
        ...
        daemon_fail(rsp, "cannot focus space due to an error with the "
                         "scripting-addition.\n");
      }
    }
    ...
    if (parse_label(rsp, get_token(&message), LABEL_DISPLAY, &label)) {
      if (label) {
        display_manager_set_label_for_display(&g_display_manager, acting_did,
                                              label);
      } else {
        if (!display_manager_remove_label_for_display(&g_display_manager,
                                                      acting_did)) {
          daemon_fail(
              rsp, "the selected display was not associated with a label!\n");
        }
      }
    }
  } else {
...
static void handle_domain_space(FILE *rsp, struct token domain, char *message) {
  ...
  struct selector selector =
      parse_space_selector(NULL, &message, acting_sid, true);
  ...
  for (; token_is_valid(command); command = get_token(&message)) {
    if (token_equals(command, COMMAND_SPACE_FOCUS)) {
      struct selector selector =
          parse_space_selector(rsp, &message, acting_sid, false);
      if (selector.did_parse && selector.sid) {
        ...
        if (result == SPACE_OP_ERROR_SAME_SPACE) {
          ...
          daemon_fail(
              rsp, "cannot focus space because mission-control is active.\n");
        } else if (result == SPACE_OP_ERROR_SCRIPTING_ADDITION) {
          daemon_fail(rsp, "cannot focus space due to an error with the "
                           "scripting-addition.\n");
        }
      ...
      if (selector.did_parse && selector.sid) {
        ...
        if (result == SPACE_OP_ERROR_SAME_SPACE) {
          ...
          daemon_fail(rsp, "cannot move space across display boundaries. use "
                           "--display instead.\n");
        } else if (result == SPACE_OP_ERROR_DISPLAY_IS_ANIMATING) {
          daemon_fail(rsp, "cannot move space because the display is in the "
                           "middle of an animation.\n");
        } else if (result == SPACE_OP_ERROR_IN_MISSION_CONTROL) {
          daemon_fail(rsp,
                      "cannot move space because mission-control is active.\n");
        } else if (result == SPACE_OP_ERROR_SCRIPTING_ADDITION) {
          daemon_fail(rsp, "cannot move space due to an error with the "
                           "scripting-addition.\n");
        }
      }
      ...
      if (selector.did_parse && selector.sid) {
        ...
        if (result == SPACE_OP_ERROR_SAME_SPACE) {
          ...
          daemon_fail(rsp, "cannot swap space because the display is in the "
                           "middle of an animation.\n");
        } else if (result == SPACE_OP_ERROR_IN_MISSION_CONTROL) {
          daemon_fail(rsp,
                      "cannot swap space because mission-control is active.\n");
        } else if (result == SPACE_OP_ERROR_SCRIPTING_ADDITION) {
          daemon_fail(rsp, "cannot swap space due to an error with the "
                           "scripting-addition.\n");
        }
      }
    } else if (token_equals(command, COMMAND_SPACE_DISPLAY)) {
      struct selector selector = parse_display_selector(
          rsp, &message, display_manager_active_display_id(), false);
      if (selector.did_parse && selector.did) {
        enum space_op_error result = space_manager_move_space_to_display(
            &g_space_manager, acting_sid, selector.did);
        if (result == SPACE_OP_ERROR_MISSING_SRC) {
          daemon_fail(rsp, "could not locate the space to act on.\n");
        } else if (result == SPACE_OP_ERROR_MISSING_DST) {
          daemon_fail(
              rsp, "could not locate the active space of the given display.\n");
        } else if (result == SPACE_OP_ERROR_INVALID_SRC) {
          daemon_fail(rsp, "acting space is the last user-space on the source "
                           "display and cannot be moved.\n");
        } else if (result == SPACE_OP_ERROR_INVALID_DST) {
          daemon_fail(
              rsp, "acting space is already located on the given display.\n");
        } else if (result == SPACE_OP_ERROR_DISPLAY_IS_ANIMATING) {
          daemon_fail(rsp, "cannot send space to display because it is in the "
                           "middle of an animation.\n");
        } else if (result == SPACE_OP_ERROR_IN_MISSION_CONTROL) {
          daemon_fail(rsp, "cannot send space to display because "
                           "mission-control is active.\n");
        } else if (result == SPACE_OP_ERROR_SCRIPTING_ADDITION) {
          daemon_fail(rsp, "cannot send space to display due to an error with "
                           "the scripting-addition.\n");
        }
      }
    } else if (token_equals(command, COMMAND_SPACE_CREATE)) {
      struct selector selector = parse_display_selector(
          rsp, &message, display_manager_active_display_id(), true);
      ...
      if (result == SPACE_OP_ERROR_MISSING_SRC) {
        ...
        daemon_fail(rsp, "cannot create space because the display is in the "
                         "middle of an animation.\n");
      } else if (result == SPACE_OP_ERROR_IN_MISSION_CONTROL) {
        daemon_fail(rsp,
                    "cannot create space because mission-control is active.\n");
      } else if (result == SPACE_OP_ERROR_SCRIPTING_ADDITION) {
        daemon_fail(rsp, "cannot create space due to an error with the "
                         "scripting-addition.\n");
      }
    } else if (token_equals(command, COMMAND_SPACE_DESTROY)) {
      struct selector selector =
          parse_space_selector(rsp, &message, acting_sid, true);
      ...
      if (result == SPACE_OP_ERROR_MISSING_SRC) {
        ...
        daemon_fail(rsp, "acting space is the last user-space on the source "
                         "display and cannot be destroyed.\n");
      } else if (result == SPACE_OP_ERROR_INVALID_TYPE) {
        daemon_fail(rsp, "cannot destroy a macOS fullscreen space.\n");
      } else if (result == SPACE_OP_ERROR_DISPLAY_IS_ANIMATING) {
        daemon_fail(rsp, "cannot destroy space because the display is in the "
                         "middle of an animation.\n");
      } else if (result == SPACE_OP_ERROR_IN_MISSION_CONTROL) {
        daemon_fail(
            rsp, "cannot destroy space because mission-control is active.\n");
      } else if (result == SPACE_OP_ERROR_SCRIPTING_ADDITION) {
        daemon_fail(rsp, "cannot destroy space due to an error with the "
                         "scripting-addition.\n");
      }
    } else if (token_equals(command, COMMAND_SPACE_EQUALIZE)) {
      struct token value = get_token(&message);
      if (!token_is_valid(value)) {
        if (!space_manager_equalize_space(&g_space_manager, acting_sid,
                                          SPLIT_X | SPLIT_Y)) {
      ...
      if (token_equals(value, ARGUMENT_COMMON_VAL_AXIS_X)) {
        if (!space_manager_mirror_space(&g_space_manager, acting_sid,
                                        SPLIT_X)) {
      ...
      char type[MAXLEN];
      struct token value = get_token(&message);
      if ((sscanf(value.text, ARGUMENT_SPACE_PADDING, type, &t, &b, &l, &r) ==
           5)) {
        if (!space_manager_set_padding_for_space(&g_space_manager, acting_sid,
                                                 parse_value_type(type), t, b,
                                                 l, r)) {
      ...
      if ((sscanf(value.text, ARGUMENT_SPACE_GAP, type, &gap) == 2)) {
        if (!space_manager_set_gap_for_space(&g_space_manager, acting_sid,
                                             parse_value_type(type), gap)) {
      ...
      if (token_equals(value, ARGUMENT_SPACE_TGL_PADDING)) {
        if (!space_manager_toggle_padding_for_space(&g_space_manager,
                                                    acting_sid)) {
      ...
      if (token_equals(value, ARGUMENT_SPACE_LAYOUT_BSP)) {
        if (space_is_user(acting_sid)) {
          space_manager_set_layout_for_space(&g_space_manager, acting_sid,
                                             VIEW_BSP);
        } else {
      ...
      if (parse_label(rsp, get_token(&message), LABEL_SPACE, &label)) {
        if (label) {
          space_manager_set_label_for_space(&g_space_manager, acting_sid,
                                            label);
        } else {
          if (!space_manager_remove_label_for_space(&g_space_manager,
                                                    acting_sid)) {
            daemon_fail(
                rsp, "the selected space was not associated with a label!\n");
          }
        }
      }
    } else {
...
static void handle_domain_window(FILE *rsp, struct token domain,
                                 char *message) {
  TIME_FUNCTION;
  ...
  struct selector selector =
      parse_window_selector(NULL, &message, acting_window, true);
  ...
  for (; token_is_valid(command); command = get_token(&message)) {
    ...
    if (token_equals(command, COMMAND_WINDOW_FOCUS)) {
      struct selector selector =
          parse_window_selector(rsp, &message, acting_window, true);
      ...
      if (acting_window) {
        window_manager_focus_window_with_raise(&acting_window->application->psn,
                                               acting_window->id,
                                               acting_window->ref);
      } else {
        daemon_fail(rsp, "could not locate the window to act on!\n");
      ...
      if (acting_window) {
        enum window_op_error result =
            window_manager_minimize_window(acting_window);
        if (result == WINDOW_OP_ERROR_CANT_MINIMIZE) {
          daemon_fail(
              rsp,
              "window with id '%d' does not support the minimize operation.\n",
              acting_window->id);
      ...
      struct selector selector =
          parse_window_selector(rsp, &message, acting_window, false);
      if (selector.did_parse && selector.window) {
        enum window_op_error result =
            window_manager_deminimize_window(selector.window);
      ...
      if (selector.did_parse && selector.did) {
        ...
        if (space_is_fullscreen(sid)) {
          ...
          window_manager_send_window_to_space(
              &g_space_manager, &g_window_manager, acting_window, sid, false);
        }
      }
    } else if (token_equals(command, COMMAND_WINDOW_SPACE)) {
      struct selector selector = parse_space_selector(
          rsp, &message, space_manager_active_space(), false);
      if (selector.did_parse && selector.sid) {
      ...
      if (selector.did_parse && selector.window) {
        enum window_op_error result =
            window_manager_swap_window(&g_space_manager, &g_window_manager,
                                       acting_window, selector.window);
        if (result == WINDOW_OP_ERROR_INVALID_SRC_VIEW) {
          ...
          daemon_fail(
              rsp, "cannot swap a window with a window in the same stack.\n");
      ...
      if (selector.did_parse && selector.window) {
        enum window_op_error result =
            window_manager_warp_window(&g_space_manager, &g_window_manager,
                                       acting_window, selector.window);
        if (result == WINDOW_OP_ERROR_INVALID_SRC_VIEW) {
          daemon_fail(rsp, "the acting window is not within a bsp space.\n");
        } else if (result == WINDOW_OP_ERROR_INVALID_DST_VIEW) {
          ...
          daemon_fail(
              rsp, "cannot warp a window with a window in the same stack.\n");
        } else if (result == WINDOW_OP_ERROR_SAME_WINDOW) {
      ...
      if (selector.did_parse && selector.window) {
        enum window_op_error result =
            window_manager_stack_window(&g_space_manager, &g_window_manager,
                                        acting_window, selector.window);
      ...
      if (selector.did_parse && selector.dir) {
        enum window_op_error result = window_manager_set_window_insertion(
            &g_space_manager, acting_window, selector.dir);
        if (result == WINDOW_OP_ERROR_INVALID_SRC_VIEW) {
          daemon_fail(rsp, "the acting window is not within a bsp space.\n");
        } else if (result == WINDOW_OP_ERROR_INVALID_SRC_NODE) {
          daemon_fail(rsp, "the acting window is not managed.\n");
      ...
      struct token value = get_token(&message);
      if ((sscanf(value.text, ARGUMENT_WINDOW_GRID, &r, &c, &x, &y, &w, &h) ==
           6)) {
        enum window_op_error result =
            window_manager_apply_grid(&g_space_manager, &g_window_manager,
                                      acting_window, r, c, x, y, w, h);
        if (result == WINDOW_OP_ERROR_INVALID_SRC_VIEW) {
      ...
      if ((sscanf(value.text, ARGUMENT_WINDOW_MOVE, type, &x, &y) == 3)) {
        enum window_op_error result = window_manager_move_window_relative(
            &g_window_manager, acting_window, parse_value_type(type), x, y);
        if (result == WINDOW_OP_ERROR_INVALID_SRC_VIEW) {
      ...
      if ((sscanf(value.text, ARGUMENT_WINDOW_RESIZE, handle, &w, &h) == 3)) {
        enum window_op_error result = window_manager_resize_window_relative(
            &g_window_manager, acting_window, parse_resize_handle(handle), w, h,
            true);
        if (result == WINDOW_OP_ERROR_INVALID_SRC_NODE) {
      ...
      if ((sscanf(value.text, ARGUMENT_WINDOW_RATIO, type, &r) == 2)) {
        enum window_op_error result = window_manager_adjust_window_ratio(
            &g_window_manager, acting_window, parse_value_type(type), r);
        if (result == WINDOW_OP_ERROR_INVALID_SRC_VIEW) {
      ...
      if (token_equals(value, ARGUMENT_WINDOW_TOGGLE_FLOAT)) {
        if (acting_window) {
          window_manager_make_window_floating(
              &g_space_manager, &g_window_manager, acting_window,
              !window_check_flag(acting_window, WINDOW_FLOAT), false);
        } else {
          daemon_fail(rsp, "could not locate the window to act on!\n");
        }
        ...
        if (acting_window) {
          window_manager_make_window_sticky(
              &g_space_manager, &g_window_manager, acting_window,
              !window_check_flag(acting_window, WINDOW_STICKY));
        } else {
          daemon_fail(rsp, "could not locate the window to act on!\n");
        }
        ...
        if (acting_window) {
          window_manager_toggle_window_zoom_parent(&g_window_manager,
                                                   acting_window);
        } else {
          daemon_fail(rsp, "could not locate the window to act on!\n");
        }
        ...
        if (acting_window) {
          window_manager_toggle_window_zoom_fullscreen(&g_window_manager,
                                                       acting_window);
        } else {
          daemon_fail(rsp, "could not locate the window to act on!\n");
        }
      ...
      if (token_equals(value, ARGUMENT_WINDOW_LAYER_BELOW)) {
        if (!window_manager_set_window_layer(acting_window, LAYER_BELOW)) {
          daemon_fail(rsp,
                      "could not change sub-layer of window with id '%d' due "
                      "to an error with the scripting-addition.\n",
                      acting_window->id);
        }
      ...
      if (value.type == TOKEN_TYPE_FLOAT &&
          in_range_ii(value.float_value, 0.0f, 1.0f)) {
        if (window_manager_set_opacity(&g_window_manager, acting_window,
                                       value.float_value)) {
          acting_window->opacity = value.float_value;
        } else {
          daemon_fail(rsp,
                      "could not change opacity of window with id '%d' due to "
                      "an error with the scripting-addition.\n",
                      acting_window->id);
        }
      } else {
        daemon_fail(
      ...
      if (!scripting_addition_order_window(acting_window->id, 1,
                                           selector_wid)) {
        daemon_fail(rsp,
                    "could not raise window with id '%d' due to an error with "
                    "the scripting-addition.\n",
                    acting_window->id);
      }
      ...
      if (!scripting_addition_order_window(acting_window->id, -1,
                                           selector_wid)) {
        daemon_fail(rsp,
                    "could not lower window with id '%d' due to an error with "
                    "the scripting-addition.\n",
                    acting_window->id);
      }
      ...
      if (token_is_valid(token) &&
          token_equals(token, ARGUMENT_WINDOW_SCRATCHPAD_RECOVER)) {
        window_manager_scratchpad_recover_windows();
      } else if (parse_label(rsp, token, LABEL_WINDOW, &label)) {
        if (label) {
          if (!window_manager_set_scratchpad_for_window(&g_window_manager,
                                                        acting_window, label)) {
            daemon_fail(rsp, "the given scratchpad is already assigned to a "
                             "different window!\n");
          }
        } else {
          if (!window_manager_remove_scratchpad_for_window(
                  &g_window_manager, acting_window, true)) {
            daemon_fail(
                rsp, "the selected window was not assigned to a scratchpad!\n");
          }
        }
      }
    } else {
...
static void handle_domain_query(FILE *rsp, struct token domain, char *message) {
  ...
  if (token_equals(command, COMMAND_QUERY_DISPLAYS)) {
    struct properties properties = parse_properties(
        rsp, get_token(&message), display_property_val, display_property_str,
        array_count(display_property_str));
    if (properties.did_error)
      return;

    struct token option =
        properties.did_parse ? get_token(&message) : properties.token;
    if (token_equals(option, ARGUMENT_QUERY_DISPLAY)) {
      uint32_t acting_did = display_manager_active_display_id();
      struct selector selector =
          parse_display_selector(rsp, &message, acting_did, true);
      ...
      daemon_fail(
          rsp,
          "unknown option '%.*s' given to command '%.*s' for domain '%.*s'\n",
          option.length, option.text, command.length, command.text,
          domain.length, domain.text);
    } else {
    ...
    struct properties properties =
        parse_properties(rsp, get_token(&message), space_property_val,
                         space_property_str, array_count(space_property_str));
    if (properties.did_error)
      return;
    ...
    struct properties properties =
        parse_properties(rsp, get_token(&message), window_property_val,
                         window_property_str, array_count(window_property_str));
    if (properties.did_error)
      return;
...
static bool parse_rule(FILE *rsp, char **message, struct rule *rule,
                       struct token token) {
  TIME_FUNCTION;
  ...
  for (; token_is_valid(token); token = get_token(message)) {
    ...
    if (string_equals(key, ARGUMENT_RULE_KEY_LABEL)) {
      ...
      if (exclusion)
        rule_set_flag(rule, RULE_APP_EXCLUDE);
      ...
      if (exclusion)
        rule_set_flag(rule, RULE_ROLE_EXCLUDE);
      ...
      if (exclusion)
        rule_set_flag(rule, RULE_SUBROLE_EXCLUDE);
      ...
      struct selector selector = parse_display_selector(
          rsp, &value, display_manager_active_display_id(), false);
      if (selector.did_parse && selector.did) {
      ...
      struct selector selector = parse_space_selector(
          rsp, &value, space_manager_active_space(), false);
      if (selector.did_parse && selector.sid) {
      ...
      if ((sscanf(value, ARGUMENT_RULE_VALUE_GRID, &rule->effects.grid[0],
                  &rule->effects.grid[1], &rule->effects.grid[2],
                  &rule->effects.grid[3], &rule->effects.grid[4],
                  &rule->effects.grid[5]) != 6)) {
        daemon_fail(rsp, "invalid value '%s' for key '%s'\n", value, key);
        did_parse = false;
      }
      ...
      if ((sscanf(value, "%f", &rule->effects.opacity) == 1) &&
          (in_range_ii(rule->effects.opacity, 0.0f, 1.0f))) {
  ...
  if (!has_filter) {
    daemon_fail(
        rsp, "missing required key-value pair 'app[!]=..' or 'title[!]=..'\n");
    did_parse = false;
  }
...
static void handle_domain_signal(FILE *rsp, struct token domain,
                                 char *message) {
  TIME_FUNCTION;
  ...
  if (token_equals(command, COMMAND_SIGNAL_ADD)) {
    ...
    for (struct token token = get_token(&message); token_is_valid(token);
         token = get_token(&message)) {
      char *key = NULL;
      char *value = NULL;
      bool exclusion = false;
      ...
      if (string_equals(key, ARGUMENT_SIGNAL_KEY_LABEL)) {
        ...
        signal.app_regex_valid =
            regcomp(&signal.app_regex, value, REG_EXTENDED) == 0;
        ...
        signal.title_regex_valid =
            regcomp(&signal.title_regex, value, REG_EXTENDED) == 0;
...
bool message_loop_begin(char *socket_path) {
  ...
  if (bind(g_message_loop.sockfd, (struct sockaddr *)&socket_address,
  ...
  fcntl(g_message_loop.sockfd, F_SETFD,
        FD_CLOEXEC | fcntl(g_message_loop.sockfd, F_GETFD));
