...
// NOTE: the defer cancels rather than throttles -- a source stepping faster than
// coalesce_ms never paints at all, so these must bypass it outright.
static inline bool focus_ring_live_follow(uint32_t wid)
{
    return __atomic_load_n(&g_focus_ring_drag_follow, __ATOMIC_RELAXED) ||
           window_manager_stepped_active(wid);
}
...
int focus_ring_easing_from_string(const char *s) {
    if (!s) return -1;
    if (strcmp(s, "linear") == 0)                                  return FOCUS_RING_EASE_LINEAR;
    if (strcmp(s, "smoothstep") == 0 || strcmp(s, "ease_in_out") == 0) return FOCUS_RING_EASE_SMOOTHSTEP;
    if (strcmp(s, "ease_in") == 0 || strcmp(s, "ease_in_quad") == 0)   return FOCUS_RING_EASE_IN_QUAD;
    if (strcmp(s, "ease_out_expo") == 0 || strcmp(s, "expo") == 0) return FOCUS_RING_EASE_OUT_EXPO;
    return -1;
}

const char *focus_ring_easing_to_string(int easing) {
    switch (easing) {
        case FOCUS_RING_EASE_LINEAR:     return "linear";
        case FOCUS_RING_EASE_SMOOTHSTEP: return "smoothstep";
        case FOCUS_RING_EASE_IN_QUAD:    return "ease_in";
        case FOCUS_RING_EASE_OUT_EXPO:   return "ease_out_expo";
...
static bool focus_ring_show_for_wid_impl(uint32_t target_wid, bool defer_when_animating)
{
    ...
    bool live_follow = focus_ring_live_follow(target_wid);
    if (coalesce_ms > 0.0f && !live_follow) {
...
// NOTE: records `rect` as the committed target so the settle re-show idempotes
// when the window lands exactly; a clamped landing differs and repaints.
void focus_ring_animate_to_rect(uint32_t wid, CGRect rect, float duration, int easing)
{
    if (!g_focus_ring.enabled) return;
    if (wid == 0) return;
    pthread_once(&g_focus_ring_dispatch_once, focus_ring_dispatch_init);
    ...
    dispatch_async(g_focus_ring_dispatch_queue, ^{
        if (!focus_ring_epoch_current(my_epoch)) {
            focus_ring_log("epoch_superseded", "src=animate_to_rect wid=%u e=%llu",
                           wid, (unsigned long long)my_epoch);
            return;
        }
        if (wid != g_focus_ring.last_target_wid) {
            focus_ring_log("retarget_skip", "wid=%u cur_target=%u",
                           wid, g_focus_ring.last_target_wid);
            return;
        }
        scripting_addition_focus_ring_retarget(wid, r.origin.x, r.origin.y,
                                               r.size.width, r.size.height,
                                               duration, easing);
        g_focus_ring.last_target_screen_rect = r;
        focus_ring_log("retarget", "wid=%u rect=(%.0f,%.0f %.0fx%.0f) dur=%.3f ez=%d",
                       wid, r.origin.x, r.origin.y, r.size.width, r.size.height,
                       duration, easing);
    });
}
