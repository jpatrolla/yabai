// Shared 9-slice mesh builder + chrome bands for the jello animator (anim.inc.m) and the
// warp-snap cover. NOTE: include before anim.inc.m — single-TU visibility.

extern CGError SLSTransactionSetWindowWarp(CFTypeRef transaction, uint32_t wid, int w, int h, float *mesh);

// left band is deliberately wide — covers the traffic-light cluster (x≈7–60)
#define WARP_SNAP_BAND_L      120.0
#define WARP_SNAP_BAND_R      24.0
#define WARP_SNAP_BAND_T      52.0
#define WARP_SNAP_BAND_B      24.0

// NOTE: bands pair-clamp against BOTH src and dst — src band == dst band IS the 1:1
// chrome pin. out = {srcX,srcY,dstX,dstY} x16; src window-local, dst global.
