...
bool window_observe(struct window *window) {
  for (int i = 0; i < array_count(ax_window_notification); ++i) {
    AXError result = AXObserverAddNotification(
        window->application->observer_ref, window->ref,
        ax_window_notification[i], window);
    if (result == kAXErrorSuccess ||
        result == kAXErrorNotificationAlreadyRegistered) {
      window->notification |= 1 << i;
    } else {
      debug("%s: %s failed with error %s\n", __FUNCTION__,
            ax_window_notification_str[i], ax_error_str[-result]);
    }
  }
...
void window_unobserve(struct window *window) {
  for (int i = 0; i < array_count(ax_window_notification); ++i) {
    if (!(window->notification & (1 << i)))
      continue;

    AXObserverRemoveNotification(window->application->observer_ref, window->ref,
                                 ax_window_notification[i]);
    window->notification &= ~(1 << i);
  }
}
...
static inline const char *window_layer(int level) {
  if (level == g_layer_below_window_level)
    return layer_str[LAYER_BELOW];
  if (level == g_layer_normal_window_level)
    return layer_str[LAYER_NORMAL];
  if (level == g_layer_above_window_level)
    return layer_str[LAYER_ABOVE];
  return "unknown";
}
...
void window_nonax_serialize(FILE *rsp, uint32_t wid, uint64_t flags) {
  ...
  if ((flags & WINDOW_PROPERTY_PID) || (flags & WINDOW_PROPERTY_APP)) {
    SLSGetWindowOwner(g_connection, wid, &connection);
    SLSConnectionGetPID(connection, &pid);
  }

  if ((flags & WINDOW_PROPERTY_DISPLAY) || (flags & WINDOW_PROPERTY_SPACE) ||
      (flags & WINDOW_PROPERTY_IS_FULLSCREEN)) {
    sid = window_space(wid);
  }

  if ((flags & WINDOW_PROPERTY_LEVEL) || (flags & WINDOW_PROPERTY_LAYER)) {
  ...
  if (flags & WINDOW_PROPERTY_IS_PIP) {
    ...
    fprintf(rsp, "\t\"is-sticky\":%s", json_bool(is_pip));
    did_output = true;
  }
...
void window_serialize(FILE *rsp, struct window *window, uint64_t flags) {
  ...
  if ((flags & WINDOW_PROPERTY_IS_VISIBLE) ||
    ...
    is_sticky = window_check_flag(window, WINDOW_STICKY) ||
                window_is_sticky(window->id);
  }
  if ((flags & WINDOW_PROPERTY_IS_VISIBLE) ||
      (flags & WINDOW_PROPERTY_IS_PIP)) {
    is_pip = window_check_flag(window, WINDOW_PIP) || window_is_pip(window);
  }
  bool did_output = false;
  fprintf(rsp, "{\n");
  ...
  if (flags & WINDOW_PROPERTY_SPLIT_TYPE) {
    ...
    fprintf(
        rsp, "\t\"split-type\":\"%s\"",
        window_node_split_str[node && node->parent ? node->parent->split : 0]);
    did_output = true;
  }
  ...
  if (flags & WINDOW_PROPERTY_SPLIT_CHILD) {
    ...
    fprintf(rsp, "\t\"split-child\":\"%s\"",
            window_node_child_str[node ? window_node_is_left_child(node)
                                             ? CHILD_FIRST
                                             : CHILD_SECOND
                                       : CHILD_NONE]);
    did_output = true;
  ...
  if (flags & WINDOW_PROPERTY_STACK_INDEX) {
    ...
    int stack_index = node && node->window_count > 1
                          ? window_node_index_of_window(node, window->id) + 1
                          : 0;
  ...
  if (flags & WINDOW_PROPERTY_IS_FULLSCREEN) {
    ...
    fprintf(rsp, "\t\"is-native-fullscreen\":%s",
            json_bool(window_check_flag(window, WINDOW_FULLSCREEN)));
    did_output = true;
  }
  ...
  if (flags & WINDOW_PROPERTY_IS_VISIBLE) {
    ...
    bool visible = ordered_in && !is_minimized &&
                   !window->application->is_hidden &&
                   (is_sticky || space_is_visible(sid));
...
CGPoint window_ax_origin(struct window *window) {
  ...
  AXUIElementCopyAttributeValue(window->ref, kAXPositionAttribute,
                                &position_ref);
...
bool window_ax_can_move(struct window *window) {
  Boolean result;
  if (AXUIElementIsAttributeSettable(window->ref, kAXPositionAttribute,
...
bool window_ax_can_resize(struct window *window) {
  Boolean result;
  if (AXUIElementIsAttributeSettable(window->ref, kAXSizeAttribute, &result) !=
      kAXErrorSuccess) {
    result = 0;
  }
  return result;
...
bool window_can_minimize(struct window *window) {
  Boolean result;
  if (AXUIElementIsAttributeSettable(window->ref, kAXMinimizedAttribute,
                                     &result) != kAXErrorSuccess) {
    result = 0;
  }
  return result;
...
bool window_is_undersized(struct window *window) {
  if (window->frame.size.width <= 500.0f)
    return true;
  if (window->frame.size.height <= 500.0f)
    return true;
  return false;
}
...
static bool window_is_minimized(struct window *window) {
  ...
  if (AXUIElementCopyAttributeValue(window->ref, kAXMinimizedAttribute,
...
bool window_is_fullscreen(struct window *window) {
  ...
  if (AXUIElementCopyAttributeValue(window->ref, kAXFullscreenAttribute,
                                    &value) == kAXErrorSuccess) {
    result = CFBooleanGetValue(value);
    CFRelease(value);
  }
...
#pragma pack(pop)
  ...
  mach_msg(&msg.header, MACH_SEND_MSG | MACH_RCV_MSG, 0x24, 0x30,
           msg.header.msgh_local_port, 0, 0);
...
CFStringRef window_role(struct window *window) { return window->role; }
...
CFStringRef window_subrole(struct window *window) { return window->subrole; }
...
static bool window_is_root(struct window *window) {
  ...
  if (AXUIElementCopyAttributeValue(window->ref, kAXParentAttribute, &value) ==
      kAXErrorSuccess) {
...
bool window_is_real(struct window *window) {
  ...
  if (!(role = window_role(window)))
    goto out;
  if (!(srole = window_subrole(window)))
    goto out;
...
bool window_is_standard(struct window *window) {
  ...
  standard_win =
      CFEqual(role, kAXWindowRole) && CFEqual(srole, kAXStandardWindowSubrole);
...
struct window *window_create(struct application *application,
                             AXUIElementRef window_ref, uint32_t window_id) {
...
void window_destroy(struct window *window) {
  window->id = 0;
  if (window->role)
    CFRelease(window->role);
  if (window->subrole)
    CFRelease(window->subrole);
  if (window->title)
    CFRelease(window->title);
