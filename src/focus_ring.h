...
// Fire-and-forget agent-path resize: animate the band to `rect` payload-side
// (no per-frame follower exists there). easing = enum animation_easing_type
// ordinal. No-op unless the ring is committed to `wid`.
...
// SAME per-VBL transaction as the slide animator, so fading during the
...
int         focus_ring_easing_from_string(const char *s);   // -1 if unknown
