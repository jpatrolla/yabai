...
void push_janky_update(uint32_t code, const void *payload, size_t size) {
    ...
    if (g_bs_port && bootstrap_look_up(g_bs_port, "git.felix.jbevent", &port) == KERN_SUCCESS) {
        struct {
            uint32_t code;
            uint8_t  payload[256]; // Adjust size accordingly
        } message;

        message.code = code;
        memcpy(message.payload, payload, size);
        debug("➡️➡️➡️ [JANKY UPDATE] CODE: %d\n", code);
        mach_send(port, &message, sizeof(uint32_t) + size);
    }
}
void push_janky_flags(uint32_t wid,
    ...
    if (g_bs_port &&
        ...
        msg.event      = 1500;      // agreed “flags-bundle” opcode
        ...
        debug("➡️➡️➡️ [JANKY FLAGS] CODE: %d\n", msg.event);
