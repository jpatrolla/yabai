...
int g_connection;
bool g_verbose;
char g_bsp_stack_indicator_position[8] = "off";
int g_bsp_stack_indicator_padding = 0;
pid_t g_pid;
...
static inline bool configure_settings_and_acquire_lock(void) {
  ...
  CGSGetConnectionPortById =
      macho_find_symbol("/System/Library/PrivateFrameworks/SkyLight.framework/"
                        "Versions/A/SkyLight",
...
static void parse_arguments(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (string_equals(opt, "--bsp-stack-indicators")) {
      char *pos = i < argc - 2 ? argv[++i] : NULL;
      char *pad = i < argc - 1 ? argv[++i] : NULL;
      if (!pos || !pad) {
        error("yabai: option '--bsp-stack-indicators' requires two arguments! "
              "(left|top|off) <px>\n");
      }
      snprintf(g_bsp_stack_indicator_position,
               sizeof(g_bsp_stack_indicator_position), "%s", pos);
      g_bsp_stack_indicator_padding = atoi(pad);
    } else {
...
int main(int argc, char **argv) {
  ...
  if (workspace_is_macos_sequoia()) {
    SLSRegisterConnectionNotifyProc(g_connection, connection_handler, 804,
                                    NULL);
  }
