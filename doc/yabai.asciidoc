...
*window_animation_transport* ['off|agent']::
    Commit path for window frame changes during animation. +
    'off': commit frames through the existing Accessibility path. +
    'agent': commit frames through the resident WindowManager agent instead; requires the agent to have been deployed with *yabai --load-wmsa*. +
    Off by default.

*window_animation_transport_hz* ['<FLOAT_SEL>']::
    Rate the daemon-side stepper drives frames at when *window_animation_transport* is 'agent'. +
    120 by default. Clamped to 540.

*window_animation_transport_autoload* ['<BOOL_SEL>']::
    Whether the daemon deploys the agent automatically. +
    Off by default.
...
*space_animation_gap* ['<FLOAT_SEL>']::
    Width (in points) of the strip left between the two spaces while they slide, so that whatever sits beneath them — the wallpaper floor, when it is up — shows through. +
    0.0 (flush) by default; a value outside the range 0 - 500 is rejected.

*space_animation_easing* ['linear|smoothstep|ease_in|ease_out_expo']::
    Curve the space slide follows. 'ease_out_expo' by default. +
    'smoothstep' also answers to 'ease_in_out', 'ease_in' to 'ease_in_quad', and 'ease_out_expo' to 'expo'.

*space_animation_animate_wallpaper* ['<BOOL_SEL>']::
    Whether each space's desktop picture rides that space during the slide. +
    'on': every picture travels with its own space, and the wallpaper floor is torn down. +
    'off': Dock's pictures are blanked and the wallpaper floor — one captured picture per display, parked below every space — shows through the slide instead. +
    Off by default. Also accepted as *space_animation_wallpaper*.

*wallpaper_floor* ['<BOOL_SEL>']::
    Build ('on') or tear down ('off') the persistent wallpaper floor directly. +
    Normally derived from *space_animation_animate_wallpaper*; set it explicitly to force a rebuild, for instance after changing the desktop picture. +
...
*window_flags_changed*::
    Triggered when the focused window's state class changes. +
    Eligible for *app*, *title* and *active* filter. +
    Passes two arguments: $YABAI_WINDOW_ID, $YABAI_WINDOW_STATE +
    $YABAI_WINDOW_STATE is one of: pip, sticky, fullscreen_zoom, parent_zoom, stack, managed, default
...
Manages the resident agent yabai injects into WindowManager.app to commit animated window frames directly, bypassing Accessibility. Enable the transport itself with *window_animation_transport*.

General Syntax
^^^^^^^^^^^^^^

yabai -m wm-agent '<COMMAND>'

COMMAND
^^^^^^^

*status*::
    Print attach state, transport state and request counters.

*reload*::
    Spawn the privileged loader and re-attach.

*detach*::
    Drop the send right to the agent; every commit falls back to Accessibility.

*warm*::
    Pre-resolve every managed window's identifier.
