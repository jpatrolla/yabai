...
*window_focus_method* ['ax|sls']::
    Specify the mechanism used to focus a window. +
    'ax': Set the front process and raise the window through the Accessibility API (kAXRaiseAction). +
    'sls': Hand focus to the WindowServer through the scripting-addition, bypassing the Accessibility API. Falls back to 'ax' if the scripting-addition is unavailable.
...
*window_animation_policy* ['true_resize|jello']::
    Presentation recipe used while a window animation is in flight. +
    'true_resize': the real window is resized every frame. +
    'jello': windows whose size changes are presented as a stretched mesh warp with a single real resize at the end. +
    System Integrity Protection must be partially disabled.
...
*focus_ring_color* ['0xAARRGGBB|auto|system']::
    Ring color. 'auto' and 'system' track the macOS accent color.

*focus_ring_width* ['<FLOAT_SEL>']::
    Band thickness in pixels.

*focus_ring_alpha* ['<FLOAT_SEL>']::
    Whole-ring translucency (0.0 - 1.0).

*focus_ring_opacity* ['<FLOAT_SEL>']::
    Alpha of the hard stroke / tint wash (0.0 - 1.0); 0.0 disables it.

*focus_ring_blur_radius* ['<FLOAT_SEL>']::
    Frost blur radius in pixels; drives the ring style: 0 is a sharp solid stroke, greater than 0 is a frosted band.

*focus_ring_blur_bleed* ['<FLOAT_SEL>']::
    Pixels of the window's own edge sampled and frosted outward; 0 disables.

*focus_ring_blur_feather* ['<FLOAT_SEL>']::
    Pixels of softening applied to the band mask's edges; 0 disables.

*focus_ring_blur_saturation* ['<FLOAT_SEL>']::
    Frost saturation (0.0 - 4.0); 1.0 leaves the frost unchanged.

*focus_ring_blur_brightness* ['<FLOAT_SEL>']::
    Frost brightness (-1.0 - 1.0); 0.0 leaves the frost unchanged.

*focus_ring_blur_contrast* ['<FLOAT_SEL>']::
    Frost contrast (0.0 - 4.0); 1.0 leaves the frost unchanged.

*focus_ring_blur_hue* ['<FLOAT_SEL>']::
    Frost hue rotation in degrees (0 - 360); 0 leaves the frost unchanged.

*focus_ring_blur_color* ['0xAARRGGBB|inherit']::
    Frost tint color; 'inherit' follows *focus_ring_color*.

*focus_ring_blur_opacity* ['<FLOAT_SEL>|inherit']::
    Frost tint alpha (0.0 - 1.0); 'inherit' follows *focus_ring_opacity*.

*focus_ring_blend_mode* ['<BLEND>']::
    Blend mode of the tint over the frost. +
...
*focus_ring_blur_stroke* ['<BOOL_SEL>']::
    Overlay a crisp stroke on the frosted band. On by default.

*focus_ring_blur_stroke_position* ['above|below']::
    Draw the stroke above or below the frosted band.

*focus_ring_blur_stroke_width* ['<FLOAT_SEL>']::
    Stroke thickness in pixels, independent of the band width.

*focus_ring_blur_stroke_color* ['0xAARRGGBB|inherit']::
    Stroke color; 'inherit' follows *focus_ring_color*.

*focus_ring_blur_stroke_opacity* ['<FLOAT_SEL>|inherit']::
    Stroke alpha (0.0 - 1.0); 'inherit' follows *focus_ring_opacity*.
