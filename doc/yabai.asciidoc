...
*window_focus_inter_display* ['<BOOL_SEL>']::
    ...
    Off by default. Directional focus of floating windows on the current space works regardless of this setting.
...
*contain_space_focus_per_display* ['<BOOL_SEL>']::
    Stop an animated space slide at a display edge (with a nudge) instead of walking across to the neighbouring display. +
    Off by default (stock behaviour: focus continues onto the next display).
...
*focus_unify* ['<BOOL_SEL>']::
    Keep yabai's tracked focus in sync with the WindowServer's key-window focus, adopting focus changes the Accessibility API does not report (e.g. native tab switches). +
    Off by default.
...
*focus_ring_animate* ['<BOOL_SEL>']::
    Ease the band on focus change and space switch; live tracking during moves stays instant.
